...
const Order = () => {
    ...
    const loadOrderData = async () => {
        try {
            ...
            const res = await axios.post(backendUrl + '/api/order/userorder',
                {}, { headers: { token } })

            if (res.data.success) {
                let allOrderItem = []
                res.data.order.map((orders) => {
            ...
            <div>
                {
                    orderData.map((item, index) => (
                        ...
                        flex flex-col md:flex-row md:items-center gap-4 md:justify-between'>
                            ...
                            <div className='md:w-1/2 flex justify-between'>
                                ...
                                <button onClick={loadOrderData} className='border rounded-sm px-4 py-2 
                                    text-sm font-medium'>Track Order</button>
                            </div>
                        </div>
                    ))
                }
